...
${CONTAINER_ON_RAM} && mount -t tmpfs -o nosuid,nodev,exec,noatime,nodiratime,mode=0755 "${MOUNT_LABEL}" "${DESTINATION}/${CONTAINER_ROOT}"
...
mount -o loop,ro,nosuid,nodev,exec,noatime,nodiratime "${DESTINATION}/${CONTAINER_ROOTFS_IMAGE}" "${DESTINATION}/${CONTAINER_ROOT}/rootfs"
[ ! "${OVERLAYFS_MOUNT_OPTIONS}" = '' ] && OVERLAYFS_MOUNT_OPTIONS="${OVERLAYFS_MOUNT_OPTIONS},"
mount -t overlay -o ${OVERLAYFS_MOUNT_OPTIONS}lowerdir=${DESTINATION}/${CONTAINER_ROOT}/rootfs,upperdir=${DESTINATION}/${CONTAINER_ROOT}/diff,workdir=${DESTINATION}/${CONTAINER_ROOT}/tmp "${MOUNT_LABEL}" "${DESTINATION}/${CONTAINER_ROOT}/mnt"
...
#################### Strip container #######################
if [ "${1}" = 'strip' ] && [ -e "${DESTINATION}/.strip.rc" ] && [ -e "${DESTINATION}/${CONTAINER_ROOT}/diff" ]; then
	echo 'Removing unnecessary files...'
	. "${DESTINATION}/.strip.rc"

	rm "${DESTINATION}/${CONTAINER_ROOT}/diff/.start-${CONTAINER_NAME}.sh"
	gzip -9 "${DESTINATION}/${CONTAINER_ROOT}/diff/.debug.txt" &
fi
############################################################
