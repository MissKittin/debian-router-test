...
for i in bootstrap-installer.rc bootstrap-installer-repos.rc; do
	echo -n "[ln] etc/${i} ./usr/local/etc"
		ln -s "$(realpath --relative-to="./usr/local/etc" "${PACKAGE_DIR}/etc/${i}")" ./usr/local/etc/${i} > /dev/null 2>&1 && echo ' [OK]' || echo ' [Fail]'
done

# Install - /usr/local/sbin
echo -n '[ln] sbin/bootstrap-installer.sh ./usr/local/sbin'
	ln -s "$(realpath --relative-to="./usr/local/sbin" "${PACKAGE_DIR}/sbin/bootstrap-installer.sh")" ./usr/local/sbin/bootstrap-installer.sh > /dev/null 2>&1 && echo ' [OK]' || echo ' [Fail]'

# Install - /usr/local/share
echo -n '[ln] share/bootstrap-installer ./usr/local/share'
	ln -s "$(realpath --relative-to="./usr/local/share" "${PACKAGE_DIR}/share/bootstrap-installer")" ./usr/local/share/bootstrap-installer > /dev/null 2>&1 && echo ' [OK]' || echo ' [Fail]'
