...
if cd /usr/local/etc/rc.local.d > /dev/null 2>&1; then
	echo -n '[ln] extras/etc/rc.local.d/fancontrol-weather.rc /usr/local/etc/rc.local.d'
		ln -s ${PACKAGE_DIR}/extras/etc/rc.local.d/PKx_fancontrol-weather.rc . > /dev/null 2>&1 && echo ' [OK]' || echo ' [Fail]'
else
