...
case "$1" in
	'start')
		...
		mkdir /run/logs2ram
		mount --bind /var/log /run/logs2ram
		mount -t tmpfs -o nosuid,noexec,nodev,mode=0755 logs2ram /var/log
		cp -rfp /run/logs2ram -T /var/log
	;;
	'stop')
		log_action_msg_pre; echo 'Syncing logs'
		mount -o remount,rw /run/logs2ram
		cp -rfup /var/log -T /run/logs2ram
		cd /run/logs2ram; find | sed 1d | while read line; do
			line=$(echo $line | cut -c2-)
			if [ ! -e "/var/log${line}" ]; then
				rm -r -f "/run/logs2ram${line}" > /dev/null 2>&1
			fi
		done
		#umount /var/log
		#umount /run/logs2ram
