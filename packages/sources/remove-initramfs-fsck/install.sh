...
# Install - /boot
cd /boot
echo -n '[ln] boot/initrd-fsck-patch.img /boot'
	ln -s ${PACKAGE_DIR}/boot/initrd-fsck-patch.img . > /dev/null 2>&1 && echo ' [OK]' || echo ' [Fail]'

# Install - /usr/local/etc/init.d
cd /usr/local/etc/init.d
echo -n '[ln] etc/init.d/remove-initramfs-fsck.sh /usr/local/etc/init.d'
	ln -s ${PACKAGE_DIR}/etc/init.d/remove-initramfs-fsck.sh . > /dev/null 2>&1 && echo ' [OK]' || echo ' [Fail]'

# Install - /etc/init.d
cd /etc/init.d
echo -n '[ln] /usr/local/etc/init.d/remove-initramfs-fsck.sh /etc/init.d'
	ln -s /usr/local/etc/init.d/remove-initramfs-fsck.sh . > /dev/null 2>&1 && echo ' [OK]' || echo ' [Fail]'

# Insserv - /etc/rcS.d
cd /etc/rcS.d
echo -n '[ln] /etc/init.d/remove-initramfs-fsck.sh /etc/rcS.d'
	ln -s ../init.d/remove-initramfs-fsck.sh ./S01remove-initramfs-fsck.sh > /dev/null 2>&1 && echo ' [OK]' || echo ' [Fail]'

# Notification
echo ''
echo ' ! Add: initrd /initrd.img /boot/initrd-fsck-patch.img :to bootloader configuration'
