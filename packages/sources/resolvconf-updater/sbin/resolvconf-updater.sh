...
# Removed debug log 01.12.2021
# Automatic file creation added, ls stderr > /dev/null and empty file bug patched 29.04.2022
...
while true; do
	...
	if [ -e '/tmp/.resolvconf-updater.pause' ]; then
		...
		if [ ! -e "${resolvconf}" ]; then
			echo -n '' > "$(readlink -f "${resolvconf}")"
			log "$(readlink -f "${resolvconf}") created"
		fi

		[ "$(cat "${resolvconf}")" = '' ] && echo '### resolv.conf created by resolvconf-updater' > "${resolvconf}"

		LOG='true' # send message to syslog only once
		#dlog "LOG=${LOG}"
	
		#dlog 'if [ ! "$(ls ${home_dir}/*.top.conf)" = "" ] => ? (default: false)'
		if [ ! "$(ls ${home_dir}/*.top.conf 2>/dev/null)" = '' ]; then
		...
		if [ ! "$(ls ${home_dir}/*.bottom.conf 2>/dev/null)" = '' ]; then
